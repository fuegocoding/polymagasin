...
def load_config(path: str = "config.toml") -> Config:
    ...
    scanner = ScannerConfig(
        edge_threshold=raw.get("edge_threshold", 0.01),
    ...
    return Config(
        ...
        sources=data.get(
            "sources", {"pinnacle": True, "stake": True, "miseonjeu": True}
        ),
        db_path=data.get("db", {}).get("path", "polyedge.db"),
        pinnacle_api_key=data.get("keys", {}).get("pinnacle_api_key", ""),
        stake_api_key=data.get("keys", {}).get("stake_api_key", ""),
        miseonjeu_api_key=data.get("keys", {}).get("miseonjeu_api_key", ""),
        polymarket_key=data.get("keys", {}).get("polymarket_key", ""),
    )
