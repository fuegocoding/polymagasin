...
def load_serializable_data():
    ...
    if _is_pg(conn):
        ...
        history_df = pd.read_sql(
            "SELECT * FROM bankroll_history ORDER BY timestamp ASC", conn
        )
...
try:
    config_dict, signals_list, pnl_sport, db_bankroll, history_df = (
        load_serializable_data()
    )
...
with st.sidebar:
    ...
    if is_live:
        ...
        st.caption(
            "Execution disabled. Set EXECUTION_ENABLED=true in Railway to go live."
        )
    ...
    st.write(
        f"**Database Mode:** {'PostgreSQL' if config_dict['database_url'] else 'SQLite'}"
    )
...
c3.metric(
    "Guaranteed Profit",
    f"${unrealized_locked_profit:,.2f}",
    help="Locked profit from arbs currently in progress.",
)
c4.metric(
    "Active Engines",
    sum(1 for v in config_dict["sources"].values() if v),
...
with tab_live:
    ...
    if pending:
        ...
        for s in sorted(pending, key=lambda x: x["edge_pct"], reverse=True):
            ...
            cost = s["suggested_size"] + (s["hedge_size"] or 0)
            profit = (
                (s["suggested_size"] / s["poly_price"]) - cost
                if s["poly_price"] > 0
                else 0
            )
            roi = (profit / cost * 100) if cost > 0 else 0
            age_td = datetime.now(timezone.utc) - s["timestamp"].replace(
                tzinfo=timezone.utc
            )
        ...
        st.dataframe(
            df_live.style.format(
                {"Total Cost": "${:.2f}", "Profit": "${:.2f}", "ROI %": "{:.2f}%"}
            ).map(
                lambda x: (
        ...
        st.success(
            f"📈 **Consolidated Guaranteed Return: ${unrealized_locked_profit:,.2f}**"
        )
    else:
...
with tab_analytics:
    if not history_df.empty:
        ...
        with col_stats:
            ...
            if pnl_sport:
                pnl_data = pd.DataFrame(
                    list(pnl_sport.items()), columns=["Sport", "Profit"]
                )
                fig_pie = px.sunburst(
                    pnl_data,
                    path=["Sport"],
        ...
        st.warning("Cumulative data is being collected.")
...
with tab_ledger:
    ...
    if all_history:
        ...
        for s in all_history:
            status_color = (
                ...
                if s["status"] == "won"
                    ...
                    "🔴"
                    if s["status"] == "lost"
                    else ("⚪" if s["status"] in ("pending", "executed") else "✖️")
                )
            )
            ledger_rows.append(
                {
        ...
        pending_for_manual = [
            s for s in signals_list if s["status"] in ("pending", "executed")
        ]
        if pending_for_manual:
            with st.expander("🛠️ Manual Settlement Tool"):
                ...
                target_id = col_id.selectbox(
                    "Select Signal ID", [s["id"] for s in pending_for_manual]
                )
                ...
                if col_go.button("Force Settle"):
                    ...
                    target_sig = next(
                        s for s in pending_for_manual if s["id"] == target_id
                    )
                    resolve_signal(
                        conn, target_id, outcome.lower(), target_sig["poly_price"]
                    )
                    if _is_pg(conn):
                        conn.close()
...
with tab_config:
    ...
    st.warning(
        "⚠️ High-level configuration is managed via environment variables for security."
    )
    with st.expander("💰 Financial Management", expanded=True):
        with st.form("sys_fin"):
            f1, f2 = st.columns(2)
            new_bank = f1.number_input(
                "Adjust Local Bankroll ($)", value=float(db_bankroll)
            )
            new_edge = (
                f2.slider(
                    "Local Threshold %",
                    0.1,
                    5.0,
                    float(config_dict["edge_threshold"] * 100.0),
                )
                / 100.0
            )
            if st.form_submit_button("Save Changes"):
                ...
                if abs(new_bank - db_bankroll) > 0.01:
                    update_bankroll(
                        conn, new_bank - db_bankroll, "Manual update via Control Panel"
                    )
    ...
    with st.expander("🗑️ Danger Zone"):
        ...
        if st.button("Wipe All Trade History"):
            ...
            try:
                ...
                conn.execute(
                    "UPDATE bankroll SET balance = ? WHERE id = 1", (current_cash,)
                )
                conn.commit()
...
st.caption(
    f"PolyEdge Elite v2.3 | Mode: {'PG' if config_dict['database_url'] else 'Local'} | Last Refresh: {datetime.now().strftime('%H:%M:%S')}"
)
